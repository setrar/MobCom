...
for m=0 : (2*N_RB_DL),
    ...
    mprime_qpsk_symb = rem(mprime,16);

    % this is r_mprime from 3GPP 36-211 6.10.1.2
    dl_cs_rs(1+k) = qpsk(1+rem(bitshift(uint32(lte_gold_table(1+Ns,1+l,1+mprime_dword)),-(2*mprime_qpsk_symb),'uint32'),4));
    fprintf('Ns %d, l %d, m %d,mprime_dword %d, mprime_qpsk_symbol %d\n',Ns,l,m,mprime_dword,-(2*mprime_qpsk_symb));
    fprintf('index = %d (k %d)\n',rem(bitshift(uint32(lte_gold_table(1+Ns,1+l,1+mprime_dword)),-(2*mprime_qpsk_symb),'uint32'),4),k);
